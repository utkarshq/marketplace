...
from saleor.graphql.account.types import User  # Ensure User is imported
...
class VendorApplication(ModelObjectType[VendorApplication]):
    id = graphene.GlobalID(required=True, description="The ID of the vendor application.")
    user = graphene.Field(User, required=True, description="The user who applied to become a vendor.")
    company_name = graphene.String(required=True, description="The company name of the vendor.")
    description = graphene.String(description="Description of the vendor.")
    is_approved = graphene.Boolean(required=True, description="Whether the vendor application is approved.")
    created_at = graphene.DateTime(required=True, description="The date and time when the application was created.")
    updated_at = graphene.DateTime(required=True, description="The date and time when the application was last updated.")

    class Meta:
        description = "Represents a vendor application."
...
class User(ModelObjectType[models.User]):
    ...
    vendor_application = graphene.Field(
        VendorApplication,
        description="The vendor application associated with the user, if any."
    )
    ...
    @staticmethod
    def resolve_vendor_application(root: models.User, info: ResolveInfo):
        return VendorApplication.objects.filter(user=root).first()
